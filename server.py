...
def print_startup_summary():
    ...
    # Обновленный статус JustinCase калькулятора
    if JUSTINCASE_AVAILABLE:
        print("  ✅ 'На всякий случай' (Риск) - РАБОТАЕТ ПОЛНОСТЬЮ")
        print("     🎯 Полная версия с актуарными таблицами")
        print("     📊 15 возрастов мужчин/женщин, 30 сроков, 4 вида НС, 2 региона КЗ")
        print("     💎 Результаты соответствуют официальному калькулятору РГС")
    else:
        print(f"  ❌ 'На всякий случай' (Риск) - ОШИБКА: {JUSTINCASE_ERROR}")
    
    print("\n🔗 ОСНОВНЫЕ ENDPOINTS:")
    print("  📋 /api/status - общий статус")
    print("  🏥 /health - health check")
    if CARE_FUTURE_AVAILABLE:
        print("  💰 /api/care-future/* - НСЖ калькулятор (полный)")
    if JUSTINCASE_AVAILABLE:
        print("  🛡️ /api/proxy/calculator/save - основной риск-калькулятор (полный)")
        print("  🔧 /api/justincase/* - расширенное API (готово)")
    
    print("\n✅ ВАЖНЫЕ ОБНОВЛЕНИЯ:")
    if JUSTINCASE_AVAILABLE:
        print("  • Полный калькулятор 'На всякий случай' успешно интегрирован")
        print("  • Актуарные таблицы из Excel загружены (11,444+ коэффициентов)")
        print("  • Результаты теперь соответствуют официальному калькулятору")
        print("  • Разница с сайтом РГС минимальна (высокая точность)")
