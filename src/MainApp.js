...
// ✅ Вся логика фонов в одно// MainApp.js - ЦЕНТРАЛИЗОВАННОЕ УПРАВЛЕНИЕ ФОНАМИ
...
// ✅ ДОБАВЛЕНО: Эффект блюра при смене фонов (0.5s) - УМЕНЬШЕННЫЙ БЛЮР 5px
...
function MainApp() {
  ...
  const [nextBlur, setNextBlur] = useState(5);        // Блюр следующего фона (5px = размытый) - УМЕНЬШЕНО В 3 РАЗА
  ...
  // ===== НАЧАЛО РАЗМЫТИЯ ТЕКУЩЕГО ФОНА (за 0.5 секунды до смены) =====
  const startBlurring = () => {
    if (!imagesLoaded || availableBackgrounds.length <= 1) return;
    
    console.log(`🌫️ Начинаем размытие текущего фона за 0.5 секунды до смены`);
    
    // Плавно размываем текущий фон за 0.5 секунды - УМЕНЬШЕНО В 2 РАЗА
    const blurStep = 5 / 5; // 5px размытия за 5 шагов (по 100ms) = 0.5 секунды
    let currentStep = 0;
    ...
    const blurInterval = setInterval(() => {
      ...
      if (newBlur >= 5) {
        ...
        console.log(`🌫️ Размытие завершено: ${5}px`);
      } else {
  ...
  const changeBackground = () => {
    ...
    // Новый фон начинает размытым и становится четким за 0.5 секунды - УМЕНЬШЕНО В 2 РАЗА
    setNextBlur(5); // Начинаем с максимального размытия 5px
    
    // Плавно убираем размытие с нового фона за 0.5 секунды
    const unblurStep = 5 / 5; // 5px размытия за 5 шагов = 0.5 секунды
    let currentStep = 0;
    
    const unblurInterval = setInterval(() => {
      currentStep++;
      const newBlur = 5 - (unblurStep * currentStep);
      ...
      if (newBlur <= 0) {
        ...
        setTimeout(() => {
          ...
          setNextBlur(5);       // Готовим для следующего перехода
  ...
  useEffect(() => {
    ...
    console.log('Запуск автоматической смены фонов: размытие через 14.5 сек, смена через 15 сек');
    
    // Размытие начинается через 14.5 секунд (за 0.5 секунды до смены) - ОБНОВЛЕНО
    const blurTimer = setInterval(() => {
      startBlurring();
    }, 14500); // ИЗМЕНЕНО: 14.5 секунд вместо 14
  ...
  const mainBackgroundStyle = {
    ...
    ...(availableBackgrounds.length > 0 && imagesLoaded ? {
      ...
      transition: 'filter 0.5s cubic-bezier(0.4, 0.0, 0.2, 1)', // ✨ УСКОРЕНО: 0.5 секунды анимация блюра
      opacity: 1 // Всегда непрозрачный
    } : {
  ...
  const nextBackgroundStyle = {
    ...
    transition: 'opacity 0.5s cubic-bezier(0.4, 0.0, 0.2, 1), filter 0.5s cubic-bezier(0.4, 0.0, 0.2, 1)', // ✨ УСКОРЕНО: 0.5 секунды анимация
    zIndex: 2, // НАД основным фоном во время перехода
    pointerEvents: 'none'
  };
  ...
  const auroraOverlayStyle = {
    ...
    background: `
      ...
      radial-gradient(circle at ${85 - (auroraOffset * 0.3)}% ${75 - (auroraOffset * 0.2)}%, 
        ...
        transparent 90%)
    `,
    zIndex: 3, // НАД всеми фонами, но под интерфейсом
    pointerEvents: 'none'
  };
